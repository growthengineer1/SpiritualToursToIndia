...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Featured journeys: progressively enhance the six-card static fallback.
    const rotator = document.querySelector('[data-featured-rotator]');
    const directoryCards = Array.from(document.querySelectorAll('.tour-directory-card'));
    if (rotator && directoryCards.length > 6) {
        const cards = Array.from(rotator.querySelectorAll('[data-featured-card]'));
        const previous = rotator.querySelector('[data-rotator-prev]');
        const next = rotator.querySelector('[data-rotator-next]');
        const status = rotator.querySelector('[data-rotator-status]');
        const catalog = directoryCards.map(card => ({
        ...
        let page = 0;
        let timer;
        const pageCount = Math.ceil(catalog.length / cards.length);
        const render = () => {
            const start = page * cards.length;
            cards.forEach((card, index) => {
                const item = catalog[(start + index) % catalog.length];
                card.href = item.href;
                card.querySelector('img').src = item.image;
                card.querySelector('img').alt = item.alt;
                card.querySelector('strong').textContent = item.title;
                card.querySelector('small').textContent = item.description;
            });
            status.textContent = `${page + 1} of ${pageCount}`;
        };
        const advance = (step) => {
        ...
        const start = () => {
            ...
            if (!window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
                timer = window.setInterval(() => advance(1), 7000);
            }
        };
